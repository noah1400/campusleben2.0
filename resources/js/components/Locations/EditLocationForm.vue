        ...
        <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
            ...
            <div class="mt-6 space-y-6 sm:mt-5 sm:space-y-5">
                <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-1 sm:col-span-2 sm:mt-0">
                        <div class="flex max-w-lg rounded-md shadow-sm">
                            <input v-model="location.name" type="text" name="name" id="name" autocomplete="name"
                ...
                <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-1 sm:col-span-2 sm:mt-0">
                        <textarea v-model="location.page_content" id="page_content" name="page_content" rows="3"
                ...
                <div class="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4 sm:border-t sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-1 sm:col-span-2 sm:mt-0">
                        <div class="flex max-w-lg">
                            <Switch name="clickable" id="clickable" v-model="location.clickable"
                                :class="[location.clickable ? 'bg-indigo-600' : 'bg-gray-200', 'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2']">
                                <span class="sr-only">Aktiv</span>
                                <span aria-hidden="true"
                                    :class="[location.clickable ? 'translate-x-5' : 'translate-x-0', 'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out']" />
                            </Switch>
                        </div>

                        <p v-if="location.clickable" :key="location.clickable" class="mt-2 text-sm text-gray-500">
...
export default {
    ...
    created() {
        this.image = this.location.image ? this.location.image : false;
        this.imageSrc = '/storage/'+this.location.image;
    },
    emits: ['close_edit'],
    methods: {
        onFileChange(e) {
        ...
        submitlocation() {
            ...
            let vm = this;
            axios.post('/l/'+vm.location.slug+'/update', formData,
                {
                    headers: {
